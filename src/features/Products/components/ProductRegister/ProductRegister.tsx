...
const ProductRegister: React.FC<IProps> = ({ labelButton = 'Cadastrar' }) => {
  ...
  const handleChangePeriod = (event: React.ChangeEvent<HTMLInputElement>) => {
    setPeriod((event.target as HTMLInputElement).value as PeriodType);
  };
            ...
            {SHOW_WEIGHT_PRICE && (
              <>
                <DecimalInput
                  ...
                  onChange={(e) =>
                    handleChange('unitaryWeight', Number(e.value))
                  }
                  label="Peso unitário"
                />
      ...
      <div>
        ...
        <Box display="flex" justifyContent="flex-end" sx={{ p: 4, gap: 2 }}>
          ...
          <Button
            ...
            disabled={
              Object.keys(errors).length > 0 || addProductMutation.isLoading
            }
